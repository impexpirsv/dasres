export default function ProjectTimeSummary({
  ...
  const percent =
    estimated === 0 ? 0 : Math.min(100, Math.round((logged / estimated) * 100));
    ...
    <section className="mt-8 rounded-3xl border border-slate-800 bg-slate-900 p-6">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-slate-800 bg-slate-950 p-5">
          ...
          <p className="mt-2 text-3xl font-bold text-white">{estimated}h</p>
        </div>

        <div className="rounded-xl border border-slate-800 bg-slate-950 p-5">
          <p className="text-xs text-slate-500">Logged</p>

          <p className="mt-2 text-3xl font-bold text-green-400">{logged}h</p>
        </div>
