...
export function ContentArticleCard({ article, category, resourcesPath = "/resources" }: { article: ContentRecord; category: KnowledgeCategory; resourcesPath?: string }) {
  ...
  return <article className="ui-card ui-card-interactive flex min-h-64 flex-col"><div className="flex flex-wrap items-center gap-2 text-xs text-slate-400"><span className="ui-badge border-cyan-400/25 bg-cyan-400/10 text-cyan-100">{category}</span><time dateTime={article.updatedDate}>{date}</time><span aria-hidden="true">·</span><span>{readingTime}</span></div><h2 className="mt-5 text-xl font-bold leading-snug sm:text-2xl"><Link href={`${resourcesPath}/${category}/${article.slug}`} className="rounded text-white hover:text-cyan-200">{article.title}</Link></h2><p className="mt-3 line-clamp-3 leading-7 text-slate-300">{article.excerpt}</p><div className="mt-auto flex flex-wrap gap-2 pt-6" aria-hidden="true">{article.tags.slice(0, 3).map((tag) => <span key={tag} className="rounded-full bg-slate-800 px-2.5 py-1 text-xs text-slate-300">{tag}</span>)}</div></article>;
}
