...
function getSecret(): string {
  ...
  throw new AppError("IDENTITY_RATE_LIMIT_NOT_CONFIGURED", 503);
}

export function keyedIdentityIdentifier(value: string): string {
  return createHmac("sha256", getSecret()).update(value, "utf8").digest("hex");
}

export function enforceIdentityLimit(key: string, limit: number, windowMs: number): void {
